...
vector<vector<float> > BYTETracker::iou_distance(vector<STrack*>& atracks, vector<STrack>& btracks, int& dist_size, int& dist_size_size)
{
	...
	vector<float> acls_id, bcls_id;

	for (int i = 0; i < atracks.size(); i++)
	{
		atlbrs.push_back(atracks[i]->tlbr);
		acls_id.push_back(atracks[i]->cls_id);
	}
	for (int i = 0; i < btracks.size(); i++)
	{
		btlbrs.push_back(btracks[i].tlbr);
		bcls_id.push_back(btracks[i].cls_id);
	}
	...
	for (int i = 0; i < _ious.size(); i++)
	{
		...
		{
			if (acls_id[i] == bcls_id[j] && acls_id[i] == 2 && btracks[j].tlwh[2] * btracks[j].tlwh[3] < this->max_ped_area) {
				if (_ious[i][j] <= 0.6 && _ious[i][j] >= 0.1) {
					_ious[i][j] = 0.48 + _ious[i][j] / 5.0;
				}
			}
...
vector<vector<float> > BYTETracker::fuse_score(vector<vector<float>>& cost_matrix, vector<STrack>& detections) {

	if (cost_matrix.empty()) {
		return cost_matrix;
	}

	vector<vector<float>> fuse_dist;

	for (int i = 0; i < cost_matrix.size(); ++i)
	{
		vector<float> _fuse_dist;
		for (int j = 0; j < detections.size(); ++j)
		{
			_fuse_dist.push_back(1 - (detections[j].score - detections[j].score * cost_matrix[i][j]));
		}
		fuse_dist.push_back(_fuse_dist);
	}
	return fuse_dist;
