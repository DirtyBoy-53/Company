...
yLine::yLine(const YShape::draw_mode_e type)
    : YShape::Shape(type)
{

}

void yLine::draw(QPainter *p, bool isdisEndPt, bool fill)
{
...
bool yLine::isAreaHavPt(const QPointF &pt)
{
    return false;
}

bool yLine::isEdgeHavPt(const QPointF &pt, int &pos)
{
    return false;
}

bool yLine::insertPoint(const QPointF &pt, int &pos)
{
    return false;
}
