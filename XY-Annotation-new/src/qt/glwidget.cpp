...
std::atomic_flag GLWidget::s_glew_init = ATOMIC_FLAG_INIT;
GLuint GLWidget::prog_yuv;
GLuint GLWidget::texUniformY;
GLuint GLWidget::texUniformU;
GLuint GLWidget::texUniformV;
...
void GLWidget::setVertices(double ratio) {
    GLfloat w = 1.0, h = 1.0;
    if (ratio < 1.0) {
        w = ratio;
    }
...
void GLWidget::setVertices(QRect rc) {
    int wnd_w = width();
    int wnd_h = height();
    if (wnd_w <= 0 || wnd_h <= 0) {
        return;
...
void GLWidget::drawTexture(HRect rc, GLTexture *tex) {
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
...
void GLWidget::drawRect(HRect rc, HColor clr, int line_width, bool bFill) {
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glOrtho(0.0, width(), height(), 0.0, -1.0, 1.0);
