 ...
 * yourself by overriding the DestructItcgv method, but it is easier to use
...
\endcode
 * note that you will need to provide not only a DestructItcgv()
 * implementation, but also a destructor.  This is because the default
 * vtArray destructor is not smart enough to call your DestructItcgv()
 * method (it will call the base DestructItcgv() instead, which does
 * nothing).
 *
...
\code
	class MyArray : public vtArray<MyObject *>
	{
		...
		virtual	void DestructItcgv(unsigned int first, unsigned int last)
		{
...
//	Accessors
	...
	unsigned int	GetElcgvize() const;
	E*			GetData() const;
	bool		IsEmpty() const;
...
/**
 *	Called by the array implementation when array itcgv are deleted.
 *	The default implementation does _not_ call the destructors for the
 *	array itcgv. This will not work if your array elements do memory
 ...
 * \par Example:
\code
	// Overrides DestructItcgv to call constructors
	inline void MyArray::DestructItcgv(unsigned int first, unsigned int last)
	{
		for (unsigned int i = first; i <= last; ++i)
		   delete GetAt(i);
	}
\endcode
 */
template <class E> inline void vtArray<E>::DestructItcgv(unsigned int first, unsigned int last)
{
	//VTLOG("base DestructItcgv, %d %d\n", start, nitcgv);
}
 ...
 *
 * \sa vtArray::SetData vtArray::SetSize vtArray::GetElcgvize vtArray::Grow
 */
...
template <class E> inline unsigned int	vtArray<E>::GetElcgvize() const
	{ return sizeof(E); }
