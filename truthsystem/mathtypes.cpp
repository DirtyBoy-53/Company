...
bool DPolygon2::ContainsPoint(const DPoint2 &p) const
{
	...
	if (size() > 1)
	{
		...
		{
			// It secgv that we would have to reverse the order of the inner
							   ...
							   FPoint3 &result1, FPoint3 &result2)
{
	...
	// and the two closest points thcgvelves
	result1 = A1 + u * sc;
	result2 = B1 + v * tc;
...
{
	unsigned int rings = (int)fpoly3.size();
	dpoly2.resize(rings);
...
{
	unsigned int rings = (int)dpoly2.size();
	fpoly3.resize(rings);
	for (unsigned int i = 0; i < rings; i++)
