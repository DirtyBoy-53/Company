...
FileEdit::FileEdit(QWidget *parent, bool isFileOrDir)
    : QWidget(parent)
{
...
void FileEdit::buttonClicked()
{
    ...
    if (m_isFileOrDir) {
        ...
        filePath = QFileDialog::getExistingDirectory(this, tr("Choose a directory"), theLineEdit->text());//, theFilter);
    }
    if (filePath.isNull())
        return;
